	...
	while 1			% Main loop: Read the input text and then classify it
		...
		for languageindex = 1:numel(languages)          % Loop through the available languages
            ...
            % Will save the probability log(P(xi|lang)) accumulated from
            % i=1 to i=languageindex
            lang_prob = 0;
			...
			for trigramindex = 1:numel(text)-2;					% Loop through all the trigrams of the input text
				...
				% *****************************************************************
                % There are 60 characters on the considered character set
                % Then there are 60^3 possible trigrams
                % Using Laplacian Smoothing to avoid zeros on the trigramcount.
                
                lang_prob = lang_prob + log((trigramcount + 1) / (total_counts(languageindex) + 60^3));
			...
			% *******************************************************************************************************************
			% scores(languageindex) is the same as P(language|x)
            % By Bayes Law and disgarding the normalization term P(x)
            % then P(language|x) = P(x|language)*P(language)
            % Considering that the a priori probabilities P(language) are the same
            % then P(language|x) = P(x|language)
            % and by Naïve Bayes:
            % P(x|language) = product of all P(xi|language)

            scores(languageindex) = lang_prob;
